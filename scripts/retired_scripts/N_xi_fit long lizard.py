    ...
    # for rho in [None, 0.5, 0.6, 0.8, 0.9, 1.0]:
        ...
        for species in ['Owl']:
            ...
            for wf_idx in range(4):
                ...
                if long_lizard:
                    results_folder = N_xi_folder + rf'Additional Figures/Long Lizard'
                ...
                if output_peak_picks:
                    ...
                    plt.tight_layout()
                    if rho==0.7:
                        os.makedirs(f'{all_results_folder}\Peak Picks', exist_ok=True)
                        plt.savefig(f'{all_results_folder}\Peak Picks\{fn_id} (Peak Picks).png', dpi=300)
                    if show_plots:
                        plt.show()
                ...
                if output_fits:
                    ...
                    for peak_freqs, peak_idxs, good_peaks in zip([good_peak_freqs, bad_peak_freqs], [good_peak_idxs, bad_peak_idxs], [True, False]):
                        ...
                        for peak_freq, peak_idx, color, subplot_idx in zip(peak_freqs, peak_idxs, colors, [1, 2, 3, 4]):
                            ...
                            if good_peaks and output_spreadsheet and not long_lizard:
