...
A_mult = False
T_xi_label = rf"$A T_\xi" if A_mult else rf"$T_\xi$"
N_xi_label = rf"$A N_\xi$" if A_mult else rf"$N_\xi$"
for pw in [False]:
    ...
    if 1:
        ...
        for loglog, subplot_idx in zip([True, False], [1, 2]):
            ...
            for k in range(len(df)):
                ...
                N_xi_mod = N_xi * A if A_mult else N_xi
                if firsts[species]:
                    label = f"{species}"
                    firsts[species] = 0
                else:
            ...
            for k in range(len(df)):
                ...
                T_xi_mod = T_xi * A if A_mult else T_xi
                if firsts[species]:
                    label = f"{species}"
                    firsts[species] = 0
                else:
        ...
        folder = os.path.join('paper_analysis', f'N_xi, T_xi vs X', A_mult_str)
