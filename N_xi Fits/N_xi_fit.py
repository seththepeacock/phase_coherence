...
for pw in [True]:
    for species in ["Human"]:
        ...
        for wf_idx in [3]:
            ...
            win_meth = {'method':'rho', 'rho':0.5, 'snapping_rhortle':0}
            ...
            suptitle = rf"[{species} {wf_idx}]   [{wf_fn}]   [{win_meth_str}]   [$\tau$={tau_s*1000:.2f}ms]   [HPF=({hpf_str})]   [$\xi_{{\text{{max}}}}={xi_max_s*1000:.0f}$ms]   [Hop = {(round(hop / fs))*1000:.2f}ms]   [{wf_len}s WF]   [{N_pd_str}]"
...
for pw in [True]:
    for species in ["Human"]:
        ...
        for wf_idx in [3]:
            ...
            wf_len = 5
            wf, wf_fn, fs, good_peak_freqs, bad_peak_freqs = get_wf(
                species=species, wf_idx=wf_idx, scale=True, hpf=hpf, wf_len=wf_len
            )             
...
print(np.max(np.abs(coherences - coherences_og.T)))