...
import pandas as pd
for power_weights in [True]:
    for species in ["Anole", "Owl", "Tokay", "Human"]:
        for rho in [0.7, -1]:
            # Initialize list for row dicts for xlsx file
            rows = []
        
        # for species in ["Anole"]:
            for wf_idx in range(2):
                print(f"Processing {species} {wf_idx} (rho={rho}, power_weights={power_weights})")
                ...
                min_xi = 0.001
                delta_xi = min_xi
                # power_weights = True # True or None
                ...
                "Set filepaths"
                power_weights_str = f"power_weights={power_weights}, " if power_weights else ""
                fn_id = rf"{species} {wf_idx}, {power_weights_str}const_Npd={const_N_pd}, dense_stft={dense_stft}, rho={rho}, snapping_rhortle={snapping_rhortle}, tau={tau*1000:.0f}ms, max_xi={max_xi}, wf_length={wf_length}s, HPF={hpf_cutoff_freq}Hz, wf={wf_fn.split('.')[0]}"
                # Calclulate Npd if we're going to hold it constant
                pkl_fn = f"{fn_id} (Coherences)"
                results_power_weights_str = f", power_weights={power_weights}" if power_weights else ""
                N_xi_folder = r"N_xi Fits/"
                pkl_folder = N_xi_folder + r"Pickles/"
                results_folder = N_xi_folder + rf"Results (rho={rho}{results_power_weights_str})/"
                ...
                if np.log2(tauS) != int(np.log2(tauS)) and fs == 44100:
                    raise ValueError(
                        "tauS is not a power of 2, but the samplerate is 44100!"
                    )
                # Get coherences if they exist
                if (
                    os.path.exists(pkl_folder + pkl_fn + ".pkl")
                    and not force_recalc_coherences
                ):
                ...
                # If they don't exist, recalculate
                else:
                    if plot_what_we_got:
                        continue
        ...
        if output_spreadsheet:
            ...
            N_xi_fitted_parameters_fn = (
                rf"{results_folder}\N_xi Fitted Parameters (rho={rho}, power_weights={power_weights_str})"
            )
