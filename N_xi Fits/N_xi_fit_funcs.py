...
def get_is_signal(coherences, f, xis, f_target_idx, f_noise=12000, sample_hw=10, z_alpha=0.05):
    ...
    return is_signal, target_coherences, xi_decayed, decayed_idx
...
def fit_peak(f, peak_idx, sample_hw, z_alpha, min_fit_xi_idx, trim_step, sigma_weighting_power, bounds, p0, coherences, xis, wf_fn, rho):
    ...
    is_signal, target_coherence, xi_decayed, decayed_idx = get_is_signal(coherences, f, xis, peak_idx, f_noise=12000, sample_hw=sample_hw, z_alpha=z_alpha)
    is_noise = ~is_signal
    
    # Curve Fit
    print(f"Fitting exp decay to {freq}Hz peak on {wf_fn} with rho={rho}")
    fit_start_idx = np.argmax(target_coherence[min_fit_xi_idx:]) + min_fit_xi_idx
