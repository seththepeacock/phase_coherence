...
from phaseco import get_coherence
from phaseco import get_stft
...
# def get_stft(wf, fs, tau=None, tauS=None, xi=None, seg_spacing=None, rho=None, win_type='boxcar', N_segs=None, cutoff_freq=None, fftshift_segs=False, return_dict=False):
...
#   """
#   if xi is None and seg_spacing is not None:
#     xi = seg_spacing
#   else:
...
def check_xi_in_num_segs(xi, seg_spacing, name):
  ...
  print(f"xi={xi}, seg_spacing={seg_spacing}, xi_in_num_segs={xi_in_num_segs}")
  return int(xi_in_num_segs)


def get_coherences(wf, fs, tauS, min_xi, max_xi, delta_xi, rho, const_N_pd=False, dense_stft=False, global_max_xi=None):
  ...
  if dense_stft:
    ...
    for i, xi in enumerate(tqdm(xis)):
      ...
      xi_in_num_segs = check_xi_in_num_segs(xi, seg_spacing, "xi_in_num_segs")
      ...
      f, stft = get_stft(wf=wf, fs=fs, tauS=tauS, seg_spacing=seg_spacing, win_type=win_type, rho=rho)
      ...
      # N_segs = N_pd + xi_in_num_segs
      # f, coherence = get_coherence(wf=wf, fs=fs, tauS=tauS, xi=xi, seg_spacing=seg_spacing, rho=rho, N_segs=N_segs)
