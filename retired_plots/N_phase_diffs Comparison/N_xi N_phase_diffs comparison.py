        ...
        for dense_stft in [True, False]:
            for const_N_phase_diffs in [True, False]:
                ...
                if dense_stft:
                    suptitle+= "   [Dense STFT]"
                    fn_id += ", seth new way"
                if const_N_phase_diffs:
                    suptitle+= "   [Constant N_phase_diffs]"
                    fn_id += ", N_phase_diffs held const"
                else:
                    suptitle+= "   [Maximize N_phase_diffs]"
                ...
                fn_id = f'dense_stft={dense_stft}, const_N_phase_diffs={const_N_phase_diffs}, {fn_id}'
                ...
                if os.path.exists(pkl_folder + pkl_fn + '.pkl') and not force_recalc_coherences:
                    ...
                    if dense_stft:
                        f, xis, coherences = colossogram_coherences(wf, fs, tauS, min_xi, max_xi, delta_xi, rho, const_N_phase_diffs)
                    else:
